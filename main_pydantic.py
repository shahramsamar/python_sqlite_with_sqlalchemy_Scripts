...
# # data entry based on user model
# # name_list = ["hassan","farajian","kamali","samar"]
# # for name in name_list:
# #     new_user = User(name=name,age=random.randint(19,33))
# #     session.add(new_user)
# # session.commit()
...
# # find one specific object with filter
# # samar = session.query(User).filter(User.name=="sama").one_or_none()
# # print(samar)


# # find based on the mathematic filter
# # users = session.query(User).filter(User.age>25,User.name.like("%ama%")).all()
# # for user in users:
# #     print(user)

# # samar = session.query(User).filter(User.id==4).one_or_none()
# # # Edit the name attribute
# # samar.name = 'ali'
# # # Commit the changes to the database
# # session.commit()

# # fetch an object and edit
# # user_obj = session.query(User).filter(User.id==13).one_or_none()
...
# # Query the user object from the database
# user_from_db = session.query(User).filter(User.id == 4).one()
